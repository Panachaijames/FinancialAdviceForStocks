// Asset-type classification and symbol conventions — the SINGLE source of truth
// for both the client and the server. Previously this logic was duplicated in
// client/src/lib/assetType.js and server/util/assetType.js and had drifted,
// causing real bugs: typing "dot" added a bogus DOT us_stock (client lacked the
// crypto ALIAS/KNOWN_CRYPTO set), toBinanceSymbol('AAPL') returned 'aaplusdt' on
// the client but '' on the server, and HKD/SGD pairs classified differently.
// Both sides now re-export from here, so they can never disagree again.
//
 ...
 * Binance quotes crypto in USDT, which we treat as USD. Non-crypto -> '' (empty).
 * @param {string} symbol
 * @returns {string}
 */
